...
def test_fit_stores_2d_training_parameters():
    params = np.hstack(
        [np.arange(10)[:, None], np.flip(np.arange(10))[:, None]]
    )
...
def test_compute_training_modal_coefficients():
    ...
    np.testing.assert_allclose(
        np.hstack(p._compute_training_modal_coefficients(m)),
        POD(rank=5).fit(m).reduce(m),
    )
...
def test_compute_training_modal_coefficients2():
    ...
    np.testing.assert_allclose(actual, expected, atol=1.0e-12, rtol=0)


def test_arrange_parametric_snapshots_shape():
    p = ParametricDMD(None, None, None)
    assert p._arrange_parametric_snapshots(np.ones((10, 20, 30))).shape == (
        20,
        300,
    )
...
def test_predict_modal_coefficients():
    ...
    np.testing.assert_allclose(
        p._predict_modal_coefficients(), expected, rtol=0, atol=1.0e-11
    )
...
def test_interpolate_missing_modal_coefficients_shape():
    ...
    assert p._interpolate_missing_modal_coefficients(
        np.random.rand(10 * 5, 40)
    ).shape == (3, 5, 40)
...
def test_forecasted_modal_coefficients_reshape():
    ...
    np.testing.assert_allclose(
        p.forecasted_modal_coefficients[0, 3], forecasted_modal_coefficients[3]
    )
    np.testing.assert_allclose(
        p.forecasted_modal_coefficients[5, 0], forecasted_modal_coefficients[20]
    )
...
def test_interpolated_modal_coefficients_reshape():
    ...
    forecasted_modal_coefficients = p._predict_modal_coefficients()
    interpolated_modal_coefficients = p._interpolate_missing_modal_coefficients(
        forecasted_modal_coefficients
    )
    np.testing.assert_allclose(
        p.interpolated_modal_coefficients[1, 3],
        interpolated_modal_coefficients[1, 3],
    )
    np.testing.assert_allclose(
        p.interpolated_modal_coefficients[2, 0],
        interpolated_modal_coefficients[2, 0],
    )
