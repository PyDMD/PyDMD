...
def test_dmd_time_5(X):
    ...
    assert np.linalg.norm(numpyfy(reconstruction[0] - reconstruction[1])) != 0
...
def test_rescale_mode_auto_same_modes(X):
    ...
    dmd_rescale_normalized_modes = np.apply_along_axis(
        normalize, 0, dmd_auto_rescale.modes
    )
    dmd_no_rescale_normalized_modes = np.apply_along_axis(
        normalize, 0, dmd_no_rescale.modes
    )

    assert_allclose(
        dmd_no_rescale_normalized_modes,
        dmd_rescale_normalized_modes,
        atol=1.0e-3,
    )
...
def test_rescale_mode_custom_same_modes(X):
    ...
    dmd_rescale = DMD(svd_rank=2, opt=True, rescale_mode=np.linspace(5, 10, 2))
    dmd_rescale.fit(X=X)
...
def test_rescale_mode_same_evolution(X):
    dmd_no_rescale = DMD(svd_rank=5, opt=True, exact=True, rescale_mode=None)
    dmd_no_rescale.fit(X=X)
    dmd_no_rescale.dmd_time["tend"] *= 2

    dmd_rescale = DMD(
        svd_rank=5, opt=True, exact=True, rescale_mode=np.linspace(5, 10, 5)
    )
    dmd_rescale.fit(X=X)
    dmd_rescale.dmd_time["tend"] *= 2

    assert_allclose(
        dmd_rescale.reconstructed_data,
        dmd_no_rescale.reconstructed_data,
        atol=1.0e-6,
    )
...
def test_rescale_mode_coefficients_count_check(X):
    dmd_rescale = DMD(svd_rank=5, opt=True, rescale_mode=np.linspace(5, 10, 6))
...
def test_predict(X):
    ...
    expected = [
        [
            0.35407111 + 0.31966903j,
            0.0581077 - 0.51616519j,
            -0.4936891 + 0.36476117j,
            0.70397844 + 0.05332291j,
            -0.56648961 - 0.50687223j,
            0.15372065 + 0.74444603j,
            0.30751808 - 0.63550106j,
            -0.5633934 + 0.24365451j,
            0.47550633 + 0.20903766j,
            -0.0985528 - 0.46673545j,
        ],
        [
            0.52924739 + 0.47782492j,
            0.08685642 - 0.77153733j,
            -0.73794122 + 0.54522635j,
            1.05227097 + 0.07970435j,
            -0.8467597 - 0.7576467j,
            0.22977376 + 1.11275987j,
            0.4596623 - 0.94991449j,
            -0.84213164 + 0.3642023j,
            0.71076254 + 0.3124588j,
            -0.14731169 - 0.69765229j,
        ],
        [
            -0.49897731 - 0.45049592j,
            -0.0818887 + 0.72740958j,
            0.69573498 - 0.51404236j,
            -0.99208678 - 0.0751457j,
            0.79832963 + 0.71431342j,
            -0.21663195 - 1.04911604j,
            -0.43337211 + 0.89558454j,
            0.79396628 - 0.3433719j,
            -0.67011078 - 0.29458785j,
            0.13888626 + 0.65775036j,
        ],
        [
            -0.2717424 - 0.2453395j,
            -0.04459648 + 0.39614632j,
            0.37889637 - 0.2799468j,
            -0.54028918 - 0.04092425j,
            0.43476929 + 0.38901417j,
            -0.11797748 - 0.57134724j,
            -0.23601389 + 0.48773418j,
            0.43239301 - 0.18699989j,
            -0.36494147 - 0.16043216j,
            0.07563728 + 0.35821j,
        ],
    ]

    assert_allclose(dmd.predict(X.T), expected, atol=1.0e-6)
...
def test_predict_exact(X):
    ...
    assert_allclose(dmd.fit(X).predict(X[:, 20:40]), expected, atol=1.0e-6)
...
def test_predict_nexact(X):
    ...
    assert_allclose(dmd.fit(X).predict(X[:, 10:30]), expected, atol=1.0e-6)
...
def test_advanced_snapshot_parameter(X):
    ...
    assert_allclose(
        dmd2.reconstructed_data.real, dmd.reconstructed_data.real, atol=1.0e-6
    )
...
def test_sorted_eigs_abs_right_modes(X):
    ...
    for idx, eig in enumerate(dmd2.eigs):
        ...
        for idx_new, eig_new in enumerate(dmd.eigs):
            if eig_new == eig:
                assert_allclose(dmd.modes.T[idx_new], mode, atol=1.0e-6)
                break
...
def test_sorted_eigs_real_right_eigs():
    ...
    for eig in dmd.eigs[1:]:
        x = complex(eig)
        assert x.real > previous.real or (
            x.real == previous.real and x.imag >= previous.imag
        )
        previous = x
...
def test_sorted_eigs_dynamics(X):
    ...
    for idx, eig in enumerate(dmd2.eigs):
        ...
        for idx_new, eig_new in enumerate(dmd.eigs):
            if eig_new == eig:
                assert_allclose(dmd.dynamics[idx_new], dynamic, atol=1.0e-6)
                break
...
def test_sorted_eigs_frequency(X):
    ...
    for idx, eig in enumerate(dmd2.eigs):
        ...
        for idx_new, eig_new in enumerate(dmd.eigs):
            if eig_new == eig:
                assert_allclose(dmd.frequency[idx_new], frq, atol=1.0e-6)
                break
...
def test_sorted_eigs_amplitudes(X):
    ...
    for idx, eig in enumerate(dmd2.eigs):
        ...
        for idx_new, eig_new in enumerate(dmd.eigs):
            if eig_new == eig:
                assert_allclose(dmd.amplitudes[idx_new], amp, atol=1.0e-6)
                break
