...
@pytest.mark.parametrize("dmd", dmds)
@pytest.mark.parametrize("X", torch_backends)
def test_tensorized_modes(dmd, X):
    X = torch.stack([X * i for i in range(1, 11)])
    dmd.fit(X=X, batch=True)
    assert_allclose(dmd.modes, [dmd.fit(X=Xi).modes for Xi in X])


@pytest.mark.parametrize("dmd", dmds)
@pytest.mark.parametrize("X", torch_backends)
def test_tensorized_eigs(dmd, X):
    X = torch.stack([X * i for i in range(1, 11)])
    dmd.fit(X=X, batch=True)
    assert_allclose(dmd.eigs, [dmd.fit(X=Xi).eigs for Xi in X])
