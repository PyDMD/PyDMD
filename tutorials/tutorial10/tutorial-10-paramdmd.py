...
t2 = np.array(
    [4 * np.pi + i * step for i in range(-N_nonpredict + 1, N_predict + 1)]
)
...
visualize_multiple(
    np.abs(testing_snapshots.real - approximation.real),
    testing_params_labels,
    figsize=(20, 2.5),
    labels_func=labels_func,
)
...
errors = np.mean(
    np.abs(testing_snapshots.real - approximation.real), axis=(1, 2)
)
